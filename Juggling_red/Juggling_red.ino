...
void setLeds(Adafruit_NeoPixel *s, int n, int p, int b)
{
  ...
  //If for any reason you want a new color CHANGE COLORS HERE!!!
  //CHANGE COLORS HERE!!!
    //Dharmesh, start here!
  //Find the colors you want on http://www.colorpicker.com
  //Top and bottom colors of each set of 6 are the darkest colors
  //The middle two colors are the brightest colors
  //Only change the hex at the end of each line, hexes start with 0x, then you change the 6 numbers that come after the 0x
  //The color picker website will give you those 6 numbers. 
  //You then have to make each set of 6 match in all of the statements below.
  //Make sure you "save as" a new color name for this file before you send the new colors to your device
  //If you don't, you'll lose the previous data, because it will save when you hit send.
  if (p == n - 3)  // p+3 pixel needs to go to 0
    {
      s->setPixelColor(p - 2, 0x57020C);
      s->setPixelColor(p - 1, 0x800000);
      s->setPixelColor(p, 0xFF3000);
      s->setPixelColor(p + 1, 0xFF3000);
      s->setPixelColor(p + 2, 0x800000);
      s->setPixelColor(0, 0x57020C);
    }
    else if (p == n - 2) // p+2 pixel needs to go to 0, p+3 pixel needs to go to 1
    {
      ...
      s->setPixelColor(p, 0xFF3000);
      s->setPixelColor(p + 1, 0xFF3000);
      s->setPixelColor(0, 0x800000);
      s->setPixelColor(1, 0x57020C);
    }
    else if (p == n - 1) // p+1 pixel at 0, p+2 pixel at 1, p+3 pixel at 2
    {
      ...
      s->setPixelColor(p - 1, 0x800000);
      s->setPixelColor(p, 0xFF3000);
      s->setPixelColor(0, 0xFF3000);
      s->setPixelColor(1, 0x800000);
      s->setPixelColor(2, 0x57020C);
    }
    // p won't ever be == s->numPixels()
    else if (p == 0)  // p-1 at numPixels-1, p-2 at numPixels-2
    {
      s->setPixelColor(n - 2, 0x57020C);
      s->setPixelColor(n - 1, 0x800000);
      s->setPixelColor(p, 0xFF3000);
      s->setPixelColor(p + 1, 0xFF3000);
      s->setPixelColor(p + 2, 0x800000);
      s->setPixelColor(p + 3, 0x57020C);
    }
    else if (p == 1)  // p-2 at numPixels-1
    {
      s->setPixelColor(n - 1, 0x57020C);
      s->setPixelColor(p - 1, 0x800000);
      s->setPixelColor(p, 0xFF3000);
      s->setPixelColor(p + 1, 0xFF3000);
      s->setPixelColor(p + 2, 0x800000);
