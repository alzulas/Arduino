...
void setLeds(Adafruit_NeoPixel *s, int n, int p, int b)
{
  ...
  if (p == n - 3)  // p+3 pixel needs to go to 0
    {
      s->setPixelColor(p - 2, 0x024725);
      s->setPixelColor(p - 1, 0x0D9E1E);
      s->setPixelColor(p, 0x43F711);
      s->setPixelColor(p + 1, 0x43F711);
      s->setPixelColor(p + 2, 0x0D9E1E);
      s->setPixelColor(0, 0x024725);
    }
    else if (p == n - 2) // p+2 pixel needs to go to 0, p+3 pixel needs to go to 1
    {
      ...
      s->setPixelColor(p, 0x43F711);
      s->setPixelColor(p + 1, 0x43F711);
      s->setPixelColor(0, 0x0D9E1E);
      s->setPixelColor(1, 0x024725);
    }
    else if (p == n - 1) // p+1 pixel at 0, p+2 pixel at 1, p+3 pixel at 2
    {
      ...
      s->setPixelColor(p - 1, 0x0D9E1E);
      s->setPixelColor(p, 0x43F711);
      s->setPixelColor(0, 0x43F711);
      s->setPixelColor(1, 0x0D9E1E);
      s->setPixelColor(2, 0x024725);
    }
    // p won't ever be == s->numPixels()
    else if (p == 0)  // p-1 at numPixels-1, p-2 at numPixels-2
    {
      s->setPixelColor(n - 2, 0x024725);
      s->setPixelColor(n - 1, 0x0D9E1E);
      s->setPixelColor(p, 0x43F711);
      s->setPixelColor(p + 1, 0x43F711);
      s->setPixelColor(p + 2, 0x0D9E1E);
      s->setPixelColor(p + 3, 0x024725);
    }
    else if (p == 1)  // p-2 at numPixels-1
    {
      s->setPixelColor(n - 1, 0x024725);
      s->setPixelColor(p - 1, 0x0D9E1E);
      s->setPixelColor(p, 0x43F711);
      s->setPixelColor(p + 1, 0x43F711);
      s->setPixelColor(p + 2, 0x0D9E1E);
