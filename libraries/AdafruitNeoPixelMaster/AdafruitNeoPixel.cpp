...
AdafruitNeoPixel::AdafruitNeoPixel(uint16_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 3), pin(p), pixels(NULL)
...
void AdafruitNeoPixel::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void AdafruitNeoPixel::show(void) {
...
void AdafruitNeoPixel::setPin(uint8_t p) {
  pinMode(pin, INPUT);
  pin = p;
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
...
void AdafruitNeoPixel::setPixelColor(
...
void AdafruitNeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t AdafruitNeoPixel::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t AdafruitNeoPixel::getPixelColor(uint16_t n) const {
  if(n >= numLEDs) {
...
uint8_t *AdafruitNeoPixel::getPixels(void) const {
  return pixels;
}

uint16_t AdafruitNeoPixel::numPixels(void) const {
  return numLEDs;
}
...
void AdafruitNeoPixel::setBrightness(uint8_t b) {
...
uint8_t AdafruitNeoPixel::getBrightness(void) const {
  return brightness - 1;
}

void AdafruitNeoPixel::clear() {
  memset(pixels, 0, numBytes);
}
